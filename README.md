...
This installs all prerequisites (`uv`, `gh`, `node`, `claude` CLI) if missing, then runs `uv sync`.
...
This prompts for your Telegram token, chat ID, and GitHub repos (in `owner/repo` format). Config is saved to `~/.minbot/config.json`.
